...
UniqueVulkanMesh VulkanTutorial::GenerateGrid1(const int size) {
	...
	float step = ((float)(size * 2.0f)) / ((float)size - 1.0f);

	for (int zz = 0; zz < size; ++zz) {
		z = min + zz * step;
		for (int xx = 0; xx < size; ++xx) {
			x = min + xx * step;
			vertexPos.push_back(Vector3(x, 0.0, z));
			vertexTexCoord.push_back(Vector2(x, z));
		}
	}
	...
	gridMesh->SetVertexPositions({ vertexPos });
	gridMesh->SetVertexTextureCoords({ vertexTexCoord });
	gridMesh->SetVertexIndices({ vertexIndex });
	gridMesh->SetDebugName("Grid");
	gridMesh->SetPrimitiveType(NCL::GeometryPrimitive::Triangles);
	gridMesh->UploadToGPU(renderer);
